...
الميزات:
  • بوت رئيسي (منصة). أي شخص بيحط توكن بوته → بصير عندو بوت خاص.
  • الرسائل بتوصل مع اسم المرسِل + زر صورة + أزرار حظر/كتم سريعة.
  • للزوار: رسالة ثابتة فيها "اعمل بوتك الخاص" + زر المطوّر (ما بتنشال).
  • تخزين دائم بقاعدة بيانات PostgreSQL (البوتات ما بتنمسح أبداً).
  • بيشتغل كمان بدون قاعدة بيانات (تخزين بملف) لو ما في DATABASE_URL.

متغيرات التشغيل:
  OWNER_BOT_TOKEN  (إجباري) — توكن البوت الرئيسي
  DATABASE_URL     (اختياري لكن مفضّل) — رابط قاعدة PostgreSQL من Render
  PLATFORM_BOT     (اختياري) — يوزر البوت الرئيسي بدون @ (default: zyh011_bot)
"""
...
# ==================================================================
#          طبقة التخزين: PostgreSQL إذا متوفر، وإلا ملف
# ==================================================================
...
if USE_DB:
    ...
    def db_all_bots():
        out = {}
        with _db() as conn:
            for row in conn.execute("SELECT owner_id, token, blocked, muted, welcome FROM bots"):
                out[row[0]] = {
                    "token": row[1],
    ...
    def db_upsert(owner_id, rec):
        with _db() as conn:
            conn.execute(
                """
                INSERT INTO bots (owner_id, token, blocked, muted, welcome)
                VALUES (%s,%s,%s,%s,%s)
                ON CONFLICT (owner_id) DO UPDATE SET
                    token=EXCLUDED.token, blocked=EXCLUDED.blocked,
                    muted=EXCLUDED.muted, welcome=EXCLUDED.welcome
                """,
                (
                    str(owner_id),
                    rec["token"],
                    json.dumps(rec["blocked"]),
                    json.dumps(rec["muted"]),
                    rec.get("welcome", ""),
                ),
            )
...
# ذاكرة العمل (نسخة سريعة بالرام، متزامنة مع التخزين)
DATA = {"bots": {}}
...
def persist(owner_id=None):
    """يحفظ سجل واحد (DB) أو الملف كامل"""
    with _lock:
        if USE_DB:
...
def _branding_kb():
    """أزرار ثابتة بتبين للزوار — ما بتنشال"""
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("🤖 اعمل بوتك الخاص", url=f"https://t.me/{PLATFORM_BOT}")],
            [InlineKeyboardButton("👨‍💻 مطوّر البوت", url=f"https://t.me/{DEVELOPER.lstrip('@')}")],
        ]
    )
...
async def child_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid == owner_id:
        ...
        # رسالة الترحيب + الأزرار الثابتة (اعمل بوتك + المطوّر)
        await update.message.reply_text(welcome, reply_markup=_branding_kb())
...
async def child_setwelcome(update, context):
    ...
    if not context.args:
        ...
        return
    rec["welcome"] = update.message.text.split(None, 1)[1]
    persist(owner_id)
...
async def child_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rec = _bot_record(owner_id)
    if rec is None:
        log.warning(f"بوت بدون سجل! owner={owner_id}")
        return
    ...
    log.info(f"📩 رسالة واردة لبوت owner={owner_id} من زائر={uid}")
    if uid in rec["blocked"]:
        log.info(f"   الزائر {uid} محظور — تجاهل")
        return
    if uid in rec["muted"]:
        log.info(f"   الزائر {uid} مكتوم — تجاهل")
        return

    text = msg.text or msg.caption or ""
    if LINK_PATTERN.search(text):
        await msg.reply_text("🚫 ممنوع إرسال روابط.", reply_markup=_branding_kb())
        return
    ...
    try:
        ...
        if pic_file_id:
            buttons.append(
                InlineKeyboardButton("🖼 الصورة", callback_data=f"pic:{_store_pic(pic_file_id)}")
            )
        ...
        log.info(f"   ✅ وصلت الرسالة لصاحب البوت {owner_id}")
        ...
        try:
            await msg.reply_text("⚠️ صار خلل بسيط، جرّب مرة تانية.")
        except Exception:
            pass
...
def _owner_menu(has_bot, bot_username=None):
    ...
    if has_bot and bot_username:
        rows.append([InlineKeyboardButton(f"🤖 بوتي: @{bot_username}", url=f"https://t.me/{bot_username}")])
...
async def owner_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ok:
        ...
        await status.edit_text(
            ...
            "افتحه واكتب /start، وخلي الناس يبعتولك — رح توصلك الرسائل هون.\n\n"
...
# ==================================================================
#            سيرفر ويب صغير (عشان Render يرضى)
# ==================================================================
...
async def run():
    ...
    if USE_DB:
        db_init()
    load_all()
    log.info(f"📦 التخزين: {'PostgreSQL' if USE_DB else 'ملف محلي'} | بوتات محفوظة: {len(DATA['bots'])}")
